...
const getInsertionMode = (editor, range) => {
  ...
  if (editor.isVoid(startBlock)) {
    ...
    return 'before';
  } if (!startInline && startBlock.text === '') {
    return 'behind';
...
Commands.insertFragmentAtRange = (editor, range, fragment) => {
  editor.withoutNormalizing(() => {
    ...
    if (
      ...
      const insertionMode = getInsertionMode(editor, range);
      const nodes = insertionMode === 'before' ? fragment.nodes : fragment.nodes.reverse();
...
const findInsertionNode = (fragment, document, documentKey) => {
  ...
  const ancestorTypesMatch = (fragmentAncestors, documentAncestors) => (
    documentAncestors.size >= fragmentAncestors.size
      && fragmentAncestors.every((fragmentNode, i) => documentAncestors.get(i).type === fragmentNode.type)
  );
  ...
  const getAncestorBlocks = (doc, key) => doc
    .getAncestors(key)
    .slice(1)
...
Commands.setBlocksAtRange = (editor, range, properties) => {
  ...
  // selection extends into the start of the end node, we actually want to
  // ignore that for UX reasons.
  const isHanging = isCollapsed === false
    && start.offset === 0
    && end.offset === 0
    && isStartVoid === false
...
Commands.unwrapBlockAtRange = (editor, range, properties) => {
  ...
  const wrappers = blocks
    .map((block) => document.getClosest(block.key, (parent) => {
  ...
  editor.withoutNormalizing(() => {
    wrappers.forEach((block) => {
      ...
      const children = block.nodes.filter((child) => blocks.some((b) => child === b || child.hasDescendant(b.key)));
...
Commands.wrapBlockAtRange = (editor, range, block) => {
  ...
  if (blocks.length === 1) {
    ...
    parent = document.getClosest(firstblock.key, (p1) => !!document.getClosest(lastblock.key, (p2) => p1 === p2));
  }
...
Commands.wrapInlineAtRange = (editor, range, inline) => {
  ...
  editor.withoutNormalizing(() => {
    ...
    if (startInline && startInline === endInline) {
      ...
      const endInner = start.key === end.key
        ? startInner
