...
class ElementInterface {
  ...
  blocks(options = {}) {
    const {
      onlyLeaves, onlyRoots, onlyTypes, match, ...rest
    } = options;
  ...
  getNodesToPathsMap() {
    const root = this;
    const map = typeof window === 'undefined' ? new global.Map() : new window.Map();
  ...
  inlines(options = {}) {
    ...
    const iterable = this.descendants({
      ...
      match: (node, path) => {
        if (onlyTypes && !onlyTypes.includes(node.type)) {
          return false;
        } if (onlyLeaves && !node.isLeafInline()) {
          return false;
        } if (onlyRoots && this.getParent(path).object !== 'block') {
          return false;
        } if (match && !match(node, path)) {
          return false;
        }
  ...
  getNextDeepMatchingNodeAndPath(path, iterator = () => true) {
    ...
    const assign = () => {
      childMatch = nextNode.object !== 'text'
  ...
  getPreviousDeepMatchingNodeAndPath(path, iterator = () => true) {
    ...
    const assign = () => {
      childMatch = previousNode.object !== 'text'
  ...
  getTextsBetweenPathPositionsAsArray(startPath, endPath) {
    ...
    this.nodes.slice(startIndex, endIndex + 1).forEach((node, i) => {
      if (node.object === 'text') {
        ...
        const childStartPath = startPath && i === 0 ? PathUtils.drop(startPath) : null;
        const childEndPath = endPath && i === endIndex - startIndex
  ...
  getMarksByTypeAsArray(type) {
    ...
    const array = this.nodes.reduce((memo, node) => (node.object === 'text'
