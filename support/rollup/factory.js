...
function configure(pkg, env, target) {
  ...
  const input = `packages/${pkg.packageName}/src/index.js`
  const deps = []
  ...
  const plugins = [
    ...
    isUmd &&
      commonjs({
        exclude: [`packages/${pkg.packageName}/src/**`],
    ...
    babel({
      include: [`packages/${pkg.packageName}/src/**`],
    }),
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${pkg.packageName}/${isProd ? pkg.umdMin : pkg.umd}`,
        exports: 'named',
        name: startCase(pkg.packageName).replace(/ /g, ''),
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${pkg.packageName}/${pkg.module}`,
          format: 'es',
          sourcemap: true,
        },
        {
          file: `packages/${pkg.packageName}/${pkg.main}`,
          format: 'cjs',
          exports: 'named',
          sourcemap: true,
        },
